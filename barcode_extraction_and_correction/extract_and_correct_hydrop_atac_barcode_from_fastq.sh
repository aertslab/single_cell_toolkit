...
#
# Purpose:
#   Read HyDrop ATAC index 2 FASTQ file with raw barcode reads and correct them
...
extract_and_correct_hydrop_atac_barcode_from_fastq () {
    local hydrop_atac_bc_whitelist_filename="${1}";
    ...
    if [ ${#@} -lt 3 ] ; then
        printf 'Usage:\n';
        printf '    extract_and_correct_hydrop_atac_barcode_from_fastq \\\n';
        printf '        hydrop_atac_bc_whitelist_file fastq_with_raw_bc_file corrected_bc_file \\\n';
        printf '        [bc_suffix] [max_mismatches] [min_frac_bcs_to_find]\n\n';
        printf 'Purpose:\n';
        printf '    Read index 2 FASTQ file with raw HyDrop ATAC barcode reads and correct them\n';
        ...
        printf 'Arguments:\n';
        printf '    hydrop_atac_bc_whitelist_file:\n';
        printf '        File with HyDrop ATAC barcode whitelist to use to correct raw barcode sequences.\n';
    ...
    if [ ! -e "${hydrop_atac_bc_whitelist_filename}" ] ; then
        printf 'Error: HyDrop ATAC barcode whitelist file "%s" could not be found.\n' "${hydrop_atac_bc_whitelist_filename}" >&2;
        return 1;
    fi
    ...
    if [ "${hydrop_atac_bc_whitelist_filename%.gz}" == "${hydrop_atac_bc_whitelist_filename}" ] ; then
        # Uncompressed file.
        read -r first_barcode < "${hydrop_atac_bc_whitelist_filename}";
    else
        # Unset pipefail.
        set +o pipefail
    ...
    # Extract and correct HyDrop ATAC barcodes in index 2 FASTQ file:
    ${CODON_RUN_CMD} \
        "${script_dir}/extract_and_correct_hydrop_atac_barcode_from_fastq.codon" \
            "${hydrop_atac_bc_whitelist_filename}" \
